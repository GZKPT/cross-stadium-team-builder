...
$('speciesList').innerHTML=D.species.slice(1).map((n,i)=>`<option value="${i+1} - ${n}"></option>`).join('');$('cup').innerHTML=D.groups.map(g=>`<option value="${g.key}"${g.key==='prime100'?' selected':''}>${g.label}</option>`).join('');$('cup').value='prime100';$('cup').onchange=resetCup;$('baseRental').onchange=()=>{team[active]=clone(group().rentals[+$('baseRental').value]);render()};$('speciesCombo').onchange=commitSpecies;
